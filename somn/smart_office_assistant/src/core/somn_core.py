...
# ═══════════════════════════════════════════════════════════════════════════════
# [v1.0 优化] 分组延迟导入系统（原v1.0扁平字典升级为分组管理）
# ═══════════════════════════════════════════════════════════════════════════════
...
class SomnCore:
    ...
    def __init__(self, base_path: str = None):
        ...
        # ── [P7] _somn_ensure.py 兼容标志（v1.0 InitFlag迁移后遗留依赖）──
    ...
    # ── 懒加载标志位掩码定义（v1.0: 类体级内部枚举） ──────
    from enum import IntFlag

    class InitFlag(IntFlag):
        """初始化标志位枚举（v1.0优化：替代16个独立bool）"""
        LLM = 1
        DUAL_MODEL = 2
        AUTONOMY_STORES = 4
        RUNTIME = 8
    ...
    # ═══════════════════════════════════════════════════════════════
    # [v1.0 核心] __getattr__ 动态代理 —— 消除 50+ 纯委托方法
    #
    ...
    def __getattr__(self, name: str):
        """
        [v1.0] 动态委托代理 —— 自动将方法调用路由到对应的 delegate/module 函数。
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            engine_count = 166  # 默认值（V1.0 含社会科学引擎）
        except Exception as e:
    ...
    # [v1.0] ~50个纯委托方法已通过 __getattr__ 动态代理消除(见上方__getattr__定义)
    ...
    def _ensure_autonomy_stores(self) -> None:
        """[v1.0: 通过 __getattr__ 代理到 delegate_ensure_autonomy_stores"""
    ...
    def _init_research_strategy_engine(self):
        ...
        try:
            ...
            self._research_strategy_init_failed = False
            logger.info("[SomnCore] 研究发现驱动策略引擎 v1.0 已初始化")
        except Exception as e:
