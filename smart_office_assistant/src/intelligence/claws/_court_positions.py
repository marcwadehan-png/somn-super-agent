...
def _zheng_cong_pair(
    ...
    system_type: SystemType = SystemType.STANDARD,
    sage_type: SageType = None,  # V4.1: 管理层sage_type支持
) -> List[Position]:
    """
    创建一正一副双岗（管理层标准配置）

    pin_level: 品级数字(2-6)，自动生成 正X品 + 从X品
    sage_type: V4.1新增，为正从双岗统一设置贤者类型
    """
...
def _build_military_positions() -> List[Position]:
    ...
    # ── 军政系统专员（v3.3.1：锦衣卫专员已独立为JU_CW_L7系列）──
...
def _build_standard_positions() -> List[Position]:
    ...
    positions.append(_p(
        ...
        description="工部侍郎，伯爵品位，工程执行总管，V4.1由德鲁克担任（西方贤者·复合型），现代管理学之父，目标管理(MBO)创始人",
        sage_type=SageType.DUAL_TYPE,
    ))
...
def _build_congress_positions() -> List[Position]:
    ...
    positions.append(_p(
        ...
        description="七人决策代表大会社会学代表，V4.1由韦伯担任（西方贤者），社会系统理论奠基人",
        sage_type=SageType.THEORIST,
    ))

    # ── 德鲁克：管理学代表（V4.1: 西方贤者进入管理层）──
    positions.append(_p(
        ...
        description="七人决策代表大会管理学代表，V4.1由德鲁克担任（西方贤者），现代管理学之父",
        sage_type=SageType.DUAL_TYPE,
    ))
...
class CourtPositionRegistry:
    ...
    def _build_all_positions(self) -> None:
        """构建所有系统的岗位树"""
        builders = [
            ("七人代表大会", _build_congress_positions),   # V4.1: 七人代表大会纳入岗位体系
            ...
            ("专员领班", _build_specialist_leaders),  # V4.1: 非锦衣卫专员领班
            ("岗位补充", _build_supplement_positions),  # V6.1: 专员岗补充(+45)
        ]
