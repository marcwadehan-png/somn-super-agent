...
class MainChainIntegration:
    ...
    def execution_engine(self):
        """延迟加载 ParallelExecutionEngine（v2.0 进度追踪/结果回收/汇总优化）"""
        if self._execution_engine is None:
            try:
                ...
                logger.info("[主线集成] ParallelExecutionEngine v2.0 加载成功")
            except Exception as e:
    ...
    def execute_parallel(
        ...
        try:
            # v2.0: 优先使用 ParallelExecutionEngine
    ...
    def _execute_with_engine(self,
        ...
        """使用 ParallelExecutionEngine 执行（v2.0）"""
